...
// func NewRequestModule(request *http.Request, reqId string, authFunc func(*http.Request) bool) *RequestModule {
func NewRequestModule(request *http.Request) *RequestModule {
...
func (req *RequestModule) body(L *lua.LState) int {
	var body string
	// Cache the body, since we can only read it once,
	// and `body()` can be called multiple times
	if req.cached {
...
func (req *RequestModule) remoteaddr(L *lua.LState) int {
	L.Push(lua.LString(httputil.GetIpAddress(req.request)))
	return 1
}
