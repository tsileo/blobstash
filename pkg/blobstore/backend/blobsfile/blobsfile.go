...
func (backend *BlobsFileBackend) Enumerate(blobs chan<- *blob.SizedBlobRef, start, stop string, limit int) error {
	...
	enum, _, err := backend.index.db.Seek(formatKey(BlobPosKey, []byte(start)))
	if err != nil {
		return err
	}
	if stop == "" {
	...
	endBytes := formatKey(BlobPosKey, []byte(stop))
	var i int
	for {
		k, _, err := enum.Next()
		if err == io.EOF {
		...
		ref := &blob.SizedBlobRef{Size: blobpos.Size(), Hash: hash}
