...
func (kvs *KvStore) Put(key, value string, version int) (*response.KeyValue, error) {
	...
	resp, err := kvs.client.DoReq("PUT", "/api/v1/vkv/key/"+key, nil, strings.NewReader(data.Encode())) //data.Encode()))
	if err != nil {
		return nil, err
	}
...
func (kvs *KvStore) Get(key string, version int) (*response.KeyValue, error) {
	resp, err := kvs.client.DoReq("GET", "/api/v1/vkv/key/"+key, nil, nil)
	if err != nil {
		return nil, err
	}
...
func (kvs *KvStore) Versions(key string, start, end, limit int) (*response.KeyValueVersions, error) {
	...
	resp, err := kvs.client.DoReq("GET", "/api/v1/vkv/key/"+key+"/versions", nil, nil)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
...
func (kvs *KvStore) Keys(start, end string, limit int) ([]*response.KeyValue, error) {
	resp, err := kvs.client.DoReq("GET", fmt.Sprintf("/api/v1/vkv/keys?start=%v&end=%v&limit=%d", start, end, limit), nil, nil)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
